      ...
      if(channel.permissionOverwrites.cache.size < 1){
        if((users && users.length > 0) || (roles && roles.length > 0)){
          ...
          message.reply({embeds :[new MessageEmbed()
            .setColor(es.color)
            .setFooter(client.getFooter(es))
            .setTitle(`<a:success:907963383066816563> **Successfully locked \`${channel.name}\` for ${users.length} Users and ${roles.length} Roles**`)
          ]});
        } else {
        ...
        if((users && users.length > 0) || (roles && roles.length > 0)){
          ...
          message.reply({embeds :[new MessageEmbed()
            .setColor(es.color)
            .setFooter(client.getFooter(es))
            .setTitle(`<a:success:907963383066816563> **Successfully locked \`${channel.name}\` for ${users ? users.length : 0} Users and ${roles ? roles.length : 0} Roles**`)
          ]});
        } else {
