...
module.exports = {
  ...
  run: async (client, message, args, cmduser, text, prefix, player, es, ls, GuildSettings) => {
    ...
    try {
      ...
      if(user) {
        if(!user.bot) return interaction?.reply({ephemeral: true, content: "<:no:904319985004978198> You can't Invite a Normal user! **IT MUST BE A BOT**"})
        ...
        let button_support_dc = new MessageButton().setStyle('LINK').setEmoji("936723374644789370").setLabel('Support Server').setURL("https://discord.gg/oryzen")//array of all buttons
